...
const calculateDates = (initialFirstDay, minDateStr, maxDateStr, initialDate) => {
  let day = {
      data: [],
      index: -1
    },
  ...
  const minDate = (0, _momentTimezone.default)(minDateStr);
  const maxDate = (0, _momentTimezone.default)(maxDateStr);
  ...
  const totalDays = (maxWeekDateUnix - minWeekDateUnix) / _constants.SECONDS_IN_DAY;
  ...
  for (let dayIndex = 0; dayIndex < totalDays; dayIndex++) {
    ...
    if (startFiveDays === currentUnix && startFiveDays <= maxDateUnix) {
      ...
      startFiveDays = currentUnix + 5 * _constants.SECONDS_IN_DAY;
    }
  }
  return {
    day,
...
const groupEventsByDate = (events = [], tzOffset) => {
  let groupedEvents = {};
...
const getDayBarStyle = (currentDate, date, theme, highlightDate = {}, dayFS, dayTU, daySO) => {
