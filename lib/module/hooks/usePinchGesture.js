...
const useZoomGesture = ({
  ...
  useAnimatedReaction(() => timeIntervalHeight.value, (current, prev) => {
  ...
  const zoomGesture = Gesture.Pinch().enabled(enabled).onStart(() => {
    ...
    const clampedHeight = clampValues(newHeight, minTimeIntervalHeight.value - 2, maxTimeIntervalHeight + 5);
    focalY.value = event.focalY;
    timeIntervalHeight.value = clampedHeight;
  }).onEnd(() => {
    const clampedHeight = clampValues(timeIntervalHeight.value, minTimeIntervalHeight.value, maxTimeIntervalHeight);
